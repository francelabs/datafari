...
/** Javadoc
 * 
 * This servlet is used to modify or download the Protwords files of the FileShare core
 * It is only called by theProtWords.html.
 * doGet is used to print the content of the file or download it.
 * doPost is used to confirm the modifications of the file.
 * The semaphore is created in the constructor.
 * 
 *
 */
...
public class ProtWords extends HttpServlet { 
	...
	private final static Logger LOGGER = Logger.getLogger(ProtWords.class
			.getName());
	/**
	 * @throws IOException 
...
String environnement = Environment.getEnvironmentVariable("DATAFARI_HOME");
		...
		content="ALL";
															//For each line
			listMutex.add(new SemaphoreLn("ALL", "Prot"));									//create a semaphore and add it to the list
		...
		try{
			if(content.equals("")){
				...
				for(SemaphoreLn sem : listMutex){							//For all the semaphores
					if (sem.getType().equals("Prot")){ //if it has the good language and type(stopwords or Synonyms for now)
						try{
							...
							PrintWriter out = response.getWriter();
							out.append("Error while reading the protwords file, please make sure the file exists and retry, if the problem persists contact your system administrator. Error code : 69014"); 	
							out.close();
							LOGGER.error("Error while reading the protwords.txt file in Protwords servlet, please make sure the file exists and is located in "+env+"/solr/solrcloud/"+server+"/conf/"+". Error 69014", e);
							if(acquiredSem != null) {
								acquiredSem.release();
							}
						}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try{
			if (request.getParameter("content")==null){  					//the user load an other page
				...
				File file ;
				String filePath =  env+"/protwords.txt";
				...
				for(SemaphoreLn sem : listMutex){ 								//Get the correct semaphore and check if it was not already released
					if (sem.getLanguage().equals(request.getParameter("language")) && sem.getType().equals("Prot")  && sem.availablePermits()<1){
						sem.release();											//Release the semaphore
					}
				}
			}
