...
public class Alerts extends HttpServlet {
   ...
   *      response) Used to print the existing alerts. Makes a request and put
  ...
  protected void doGet(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
    try {
      ...
      try {
        final List<Properties> alerts = AlertDataService.getInstance().getAlerts(); // Get
                                                                                    // all
                                                                                    // the
                                                                                    // existing
                                                                                    // Alerts
        for (final Properties alert : alerts) { // Get the next Alert
          if (!request.getParameter("keyword").equals("")) { // If the user have
                                                             // typed something
                                                             // in the search
                                                             // field
            if (alert.get("keyword").equals(request.getParameter("keyword"))) { // then
                                                                                // only
                                                                                // the
                                                                                // Alerts
                                                                                // with
                                                                                ...
                                                                                // the
                                                                                // Json
              if (request.getRemoteUser().equals(alert.get("user")) || request.isUserInRole("SearchAdministrator")) { // Only
                                                                                                                      // the
                                                                                                                      // Alerts
                                                                                                                      // with
                                                                                                                      // the
                ...
                alertsArray.add(put(alert, request.isUserInRole("SearchAdministrator"))); // put
                                                                                          // the
                                                                                          // jsonObject
                                                                                          // in
                                                                                          // an
        ...
        superJson.put("length", i); // Put the number of alerts at the end of
                                    // the JSON object (handy to print the
   ...
   *      response) Used to delete/add/edit an Alert Directly change the
    ...
    try {
      ...
      try {
        if (request.getParameter("_id") != null) {
          Alert.deleteAlert(request.getParameter("_id"));// Deleting part
                                                         // //Execute the query
                                                         // in the collection
        }
        if (request.getParameter("keyword") != null) {
          final Properties alert = new Properties();// Adding part
          for (final Enumeration<String> e = request.getParameterNames(); e.hasMoreElements();) { // For
                                                                                                  // all
                                                                                                  // the
                                                                                                  // parameters
                                                                                                  // passed,
            ...
            if (!elem.equals("_id")) { // Do not put the _id manually so if the
                                       // parameter is "_id" we do not put it
                                       // in,
              alert.put(elem, request.getParameter(elem)); // otherwise there
                                                           // will be an
                                                           // exception at the
                                                           // 2nd modification
                                                           // or at a removal
