...
public class UserManagementLauncher implements ServletContextListener {
  ...
  private static Logger LOGGER = LogManager.getLogger(UserManagementLauncher.class.getName());
  ...
  public void contextInitialized(final ServletContextEvent arg0) {
    try {
      ...
      if (!UserDataService.getInstance().isInBase("admin")) {
        ...
        final User user = new User("admin", DatafariMainConfiguration.getInstance().getProperty(DatafariMainConfiguration.TEMP_ADMIN_PASSWORD));
    ...
    // Keep TEMPADMINPASSWORD in datafari.properties : should be changed by user
    // anyway
