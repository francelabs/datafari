...
AjaxFranceLabs.AdvancedSearchWidget = AjaxFranceLabs.AbstractWidget.extend({
	...
	// Be careful : This number is never decreased, even if a field is deleted by the user or removed by the onchange method of the dropdown box to select a field !
	...
	// The ones that are deleted by the user or removed by the onchange method of the dropdown box to select a field are and must be removed from this list !
	fieldsList : [],
	...
	// Represents the list of fieldnames that have a specific Solr field for exact match queries associated with the name of this specific Solr field. 
	...
	buildWidget : function() {
		...
		$.get("./GetExactFields", function(data) {
			if(data.code == 0) {
				...
				// Construct the exactFieldsList using a fieldname as key and [fieldname]_exact as the corresponding value that represents the Solr fieldname for exact match queries
			...
			if(fields != null && fields != undefined && fields != "") {
				...
				for(var cpt=0; cpt<fields.length; cpt++) {
					...
					if(fieldType == "text") {
						...
						// If the field type is not text but a number or a date then the filter value can only have two shapes : a single value or a range of values 
						var fromValue = null;
						var toValue = null;
						...
						if(fieldExpression.startsWith('[')) {
							...
							// Thus to simplify the treatments of the advanced search, the direct value is converted to a range filter with the same value as from/to values
	...
	extractFilterFromText : function(text, negativeExpression, isBasicSearchText) {
		...
		if(!text.startsWith("(") && !isBasicSearchText) {
			...
			tempFieldExpression = text; // reinit the temp variable with the text variable which has not been modified by the previous OR expression extraction
	...
	// otherwise builds the dropdown box and initialize it with the provided fielName param and trigger the constructFilter method that will build the correct filters in the UI according to the field type and eventually fill the inputs with provided values if any
	addField : function(fieldName, values, operator) {
		...
		select.change(function() {
			...
			var selectSave = select.detach(); // Detach the dropdown box of fields as it is saved it with all the assigned attributes and javascript methods 
			var selectOperatorSave = selectOperator.detach(); // Detach the dropdown box of operator as it is saved it with all the assigned attributes and javascript methods
			...
			var newOperator = div.find('.select-operator'); // Retrieve the correct operator dropdown box because the reference changes after each detach/attach jquery methods
