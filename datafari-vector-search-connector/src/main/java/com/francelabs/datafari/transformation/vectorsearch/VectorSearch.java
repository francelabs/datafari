...
*/
package com.francelabs.datafari.transformation.vectorsearch;

import java.io.*;
import java.util.*;

import com.francelabs.datafari.transformation.vectorsearch.model.VectorSearchSpecification;
 ...
 *
 */
public class VectorSearch extends BaseTransformationConnector {

  public static final String _rcsid = "@(#)$Id: "+ VectorSearch.class.getName() + " $";
  ...
  protected static final String ACTIVITY_VECTOR_SEARCH = "VECTOR SEARCH";
  protected static final String SEQNUM = "SEQNUM";

  protected static final String[] activitiesList = new String[] { ACTIVITY_VECTOR_SEARCH };
  private static final Logger LOGGER = LogManager.getLogger(VectorSearch.class.getName());
  ...
  public int addOrReplaceDocumentWithException(final String documentURI, final VersionContext pipelineDescription, final RepositoryDocument document, final String authorityNameString,
    ...
    final VectorSearchSpecification spec = new VectorSearchSpecification(pipelineDescription.getSpecification(), getConfiguration());
    Boolean enableVectorSearch = spec.getBooleanProperty(VectorSearchConfig.NODE_ENABLE_VECTOR_SEARCH, false);
    Boolean enableEmbeddingsAtIndexing = spec.getBooleanProperty(VectorSearchConfig.NODE_ENABLE_EMBEDDINGS_AT_INDEXING, false);
    ...
    try {
      ...
      activities.recordActivity(startTime, ACTIVITY_VECTOR_SEARCH, document.getBinaryLength(), documentURI, "KO", e.getLocalizedMessage());
      return activities.sendDocument(documentURI, document);
    }

    activities.recordActivity(startTime, ACTIVITY_VECTOR_SEARCH, document.getBinaryLength(), documentURI, "OK", "");
    return activities.sendDocument(documentURI, document);
  ...
  public void outputSpecificationHeader(final IHTTPOutput out, final Locale locale, final Specification spec, final int connectionSequenceNumber, final List<String> tabsArray)
    ...
    tabsArray.add(Messages.getString(locale, "vectorSearch.TabName"));
  ...
  public String processSpecificationPost(final IPostParameters variableContext, final Locale locale, final Specification spec, final int connectionSequenceNumber) throws ManifoldCFException {
    ...
    VectorSearchSpecification specification = new VectorSearchSpecification();
    Map<String, Object> params = specification.getSpec();
  ...
  private static void addChildToSpec(IPostParameters variableContext, Specification spec, String fieldName, String nodeName) {
    ...
    node.setAttribute(VectorSearchConfig.ATTRIBUTE_VALUE, Objects.requireNonNullElse(value, ""));
  ...
  protected static void fillInSpecificationMap(final Map<String, Object> paramMap, final Specification os) {
    ...
    VectorSearchSpecification specification = new VectorSearchSpecification(os, new ConfigParams());

    for (Map.Entry<String, Object> entry : specification.getSpec().entrySet()) {
