			...
			for(int i = 0 ; i < lines.length ; i++){				//for each line
				try{
					if(lines[i].startsWith("ALERTS")){			
						if(lines[i].replace("\\", "").substring(lines[i].indexOf("o")).equals("on")){
						...
						json.put("hourlyDate", lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", ""));
					else if(lines[i].startsWith("DAILYDELAY"))		//Gets the date for the daily alerts
						json.put("dailyDate", lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", ""));
					else if(lines[i].startsWith("WEEKLYDELAY"))		//Gets the date for the weekly alerts
						json.put("weeklyDate", lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", ""));
					else if(lines[i].startsWith("HOST"))			//Gets the address of the host
						json.put("host", lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", ""));
					else if(lines[i].startsWith("PORT"))			//Gets the port
						json.put("port", lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", ""));
					else if(lines[i].startsWith("DATABASE"))		//Gets the name of the database
						json.put("database", lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", ""));
					else if(lines[i].startsWith("COLLECTION"))		//Gets the name of the collection
						json.put("collection", lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", ""));
					else if(lines[i].startsWith("Hourly")){		//Gets the previous execution date for the hourly alerts
						DateTime next = new DateTime(formatter.parseDateTime(lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", ""))).plusHours(1);
						DateTime previous = new DateTime(formatter.parseDateTime(lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", "")));
						...
						DateTime next = new DateTime(formatter.parseDateTime(lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", ""))).plusDays(1);
						DateTime previous = new DateTime(formatter.parseDateTime(lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", "")));
						...
						DateTime next = new DateTime(formatter.parseDateTime(lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", ""))).plusWeeks(1);
						DateTime previous = new DateTime(formatter.parseDateTime(lines[i].substring(lines[i].indexOf("=")+1, lines[i].length()).replace("\\", "")));
