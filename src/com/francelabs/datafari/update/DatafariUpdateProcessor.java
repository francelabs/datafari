...
public class DatafariUpdateProcessor extends UpdateRequestProcessor {
	...
	public void processAdd(AddUpdateCommand cmd) throws IOException {
		...
		if( RealTimeGetComponent.getInputDocument(core, indexedID) != null){			//If the doc has already been indexed we add the previous value of category to the new doc
			doc.addField("categorie", RealTimeGetComponent.getInputDocument(core, indexedID).getFieldValue("categorie"));
		}
		...
		String mimeType = (String) doc.get("ignored_content_type")
				.getFirstValue();
		...
		if (url.startsWith("http")) {
			if (doc.get("extension").getValue().equals("")){
				doc.setField("extension", "web_page");
			}
		}
