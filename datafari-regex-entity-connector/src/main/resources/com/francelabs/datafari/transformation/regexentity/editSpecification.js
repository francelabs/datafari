...
<script type="text/javascript">
  ...
  function s${SEQNUM}_addMetaRegex()
  {
    ...
    if (editjob.s${SEQNUM}_regexField.value == "")
    {
      alert("$Encoder.bodyEscape($ResourceBundle.getString('RegexEntity.NoRegexSpecified'))");
  ...
  function s${SEQNUM}_deleteMetaRegex(i)
  {
    // Set the operation
    eval("editjob.s${SEQNUM}_metaRegex_op"+i+".value=\"Delete\"");
    // Submit
    if (editjob.s${SEQNUM}_metaRegex_count.value==i)
      postFormSetAnchor("s${SEQNUM}_metaRegex");
    else
      postFormSetAnchor("s${SEQNUM}_metaRegex"+i)
    // Undo, so we won't get two deletes next time
    eval("editjob.s${SEQNUM}_metaRegex_op"+i+".value=\"Continue\"");
  }
