...
ZOOPIDFILE="${DATAFARI_HOME}/pid/zookeeper-solr.pid"
...
ZOO_LOG_FILE=zookeeper-solr.log
_ZOO_DAEMON_OUT="$ZOO_LOG_DIR/zookeeper-solr.out"
...
start)
    ...
    nohup "$JAVA" "-Duser.timezone=UTC" $ZOO_DATADIR_AUTOCREATE "-Dzookeeper.log.dir=${ZOO_LOG_DIR}" \
    "-Dzookeeper.log.file=${ZOO_LOG_FILE}" "-Dlogback.configurationFile=$ZOOCFGDIR/logback.xml" \
