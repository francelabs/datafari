...
import com.francelabs.datafari.utils.RequestParams;
 ...
 */
public class DatafariProxy extends HttpServlet {
	...
	private static final List<String> allowedHandlers = Arrays.asList("/select", "/suggest");
	...
	public void init(ServletConfig servletConfig) throws ServletException {
		...
		String domain = servletConfig.getInitParameter("domain");
		if (domain != null) {
			this.domain = domain;
		}
	...
	protected void service(HttpServletRequest servletRequest,
		...
		if (!isHandlerAllowed(servletRequest)){
		    ...
		    for (String handler : allowedHandlers){
			    out.print(handler+" ");
		    }
		...
		RequestParams addParams = new RequestParams();
		
		if (servletRequest.getUserPrincipal() != null){
			String AuthenticatedUserName = servletRequest
					.getUserPrincipal().getName().replaceAll("[^\\\\]*\\\\" , "");
			if (!domain.equals("")){
				AuthenticatedUserName += "@" + domain;
			}
			addParams.put("AuthenticatedUserName", AuthenticatedUserName);
		}
	...
	private boolean isHandlerAllowed(HttpServletRequest servletRequest) {

		String pathInfo = servletRequest.getPathInfo();
		String handler = pathInfo.substring(pathInfo.lastIndexOf("/"), pathInfo.length());
	...
	private String rewriteUrlFromRequest(HttpServletRequest servletRequest, RequestParams addParams) {
		...
		if (queryString != null && queryString.length() > 0) {
			...
			RequestParams requestParams = new RequestParams(queryNoFrag);
			
			//Override params with addParams parameters
			requestParams.putAll(addParams);
			

			queryNoFrag = requestParams.toQueryString();
