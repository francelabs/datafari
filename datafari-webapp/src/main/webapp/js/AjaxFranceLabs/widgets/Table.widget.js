...
AjaxFranceLabs.TableWidget = AjaxFranceLabs.AbstractFacetWidget.extend({
  ...
  sortBy : function(sort) {
    ...
    case 'ZtoA':
      /*
       * elm.find('ul').prepend($('ul li .filterFacetCheck input:checked',
       * elm).parents('li')); elm.find('ul li .filterFacetCheck
       * input:checked').parents('li').each(function() { var $this = this;
       * $(this).nextAll().each(function() { if ($('.filterFacetCheck
       * .filterFacetLinkValue', $this).text().toLowerCase() <
       * $('.filterFacetCheck .filterFacetLinkValue', this).text().toLowerCase() &&
       * $('.filterFacetCheck input', this).attr('checked') == 'checked')
       * $(this).after($($this).detach()); }); }); elm.find('ul li
       * .filterFacetCheck input:not(:checked)').parents('li').each(function() {
       * var $this = this; $(this).nextAll().each(function() { if
       * ($('.filterFacetCheck .filterFacetLinkValue',
       * $this).text().toLowerCase() < $('.filterFacetCheck
       * .filterFacetLinkValue', this).text().toLowerCase())
       * $(this).after($($this).detach()); }); });
       */
