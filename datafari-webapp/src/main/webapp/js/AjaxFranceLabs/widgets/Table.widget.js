 ...
 * under the License.
 ****************************************************************************************************************************************************/
AjaxFranceLabs.TableWidget = AjaxFranceLabs.AbstractFacetWidget
    .extend({
      ...
      buildWidget : function() {
        ...
        if (this.pagination === true) {
          this.pagination = new AjaxFranceLabs.PagerModule({
            ...
            updateList : function() {
              if (this.nbPage > 1) {
                $(this.source).children().css('display', 'none').slice(this.pageSelected * this.nbElmToDisplay,
                    (this.pageSelected + 1) * this.nbElmToDisplay).css('display', this.display);
        ...
        if (data.length == 0) {
          ...
          for (var i = 0; i < max; i++) {
            ...
            if (decodedName !== '') {
              ...
              elm.find('ul li:last .filterFacetCheck').append('<input type="checkbox" value="' + decodedName + '"/>');
              elm.find('ul li:last .filterFacetCheck input').attr('id', self.id + "-" + data[i].name);
              if (self.selectionType != 'OR' && this.manager.store.find('fq', self.field + ':' + AjaxFranceLabs.Parameter.escapeValue(decodedName))) {
                elm.find('ul li:last .filterFacetCheck input').attr('checked', 'checked').parents('li').addClass('selected');
              } else if (this.manager.store.find('fq', new RegExp(self.field + ':' + AjaxFranceLabs.Parameter.escapeValue(decodedName) + '[ )]'))) {
                elm.find('ul li:last .filterFacetCheck input').attr('checked', 'checked').parents('li').addClass('selected');
              }
              ...
              if (elm.find('ul li:last .filterFacetCheck input').attr('checked') == 'checked') {
                elm.find('ul li:last .filterFacetCheck label').attr('for', self.id + "-" + data[i].name).append(
                    '<span class="checkboxIcon fa fa-check-square-o">&nbsp;</span>' + '<span class="filterFacetLinkValue">'
                        + AjaxFranceLabs.tinyString(label, self.displayChars) + '</span>').append(
      ...
      sortBy : function(sort) {
        ...
        switch (sort) {
          ...
          case 'ZtoA':
             ...
             * $('.filterFacetCheck input', this).attr('checked') == 'checked') $(this).after($($this).detach()); }); }); elm.find('ul li
             * .filterFacetCheck input:not(:checked)').parents('li').each(function() { var $this = this; $(this).nextAll().each(function() { if
          ...
          case 'occurences':
            elm.find('ul li').each(
                function() {
                  ...
                  $(this).nextAll().each(
                      function() {
                        if (parseInt($('.filterFacetCheck .filterFacetLinkCount span', $this).text()) < parseInt($(
