...
AjaxFranceLabs.TableFacetQueriesWidget = AjaxFranceLabs.AbstractFacetWidget.extend({
  ...
  buildWidget : function() {
    ...
    if (this.name != null) {
      elm.prepend('<div class="facetName"></div>').find('.facetName').append('<i class="fas fa-chevron-down"></i>').append(
  ...
  update : function() {
    var self = this, data = this.assocTags(this.manager.response.facet_counts.facet_queries), max = (data.length > this.maxDisplay) ? this.maxDisplay
