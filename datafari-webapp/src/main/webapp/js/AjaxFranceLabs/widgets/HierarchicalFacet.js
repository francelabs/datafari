 ...
 * under the License.
 ****************************************************************************************************************************************************/
AjaxFranceLabs.HierarchicalFacetWidget = AjaxFranceLabs.AbstractFacetWidget
    .extend({
      ...
      buildWidget : function() {
        ...
        if (this.name != null) {
          elm.prepend('<div class="facetName">').find('.facetName').append('<i class="fas fa-chevron-down"></i>').append(
        ...
        elm.find('.facetSort').append('<label></label>').find('label').append(window.i18n.msgStore['sortFacet']).append('<select></select>').find(
            'select').append('<option><span>A/Z</span></option>').append('<option><span>Z/A</span></option>').append(
            '<option><span>Occurences</span></option>').change(function(event) {
      ...
      update : function() {
        var self = this, data = this.assocTags(this.manager.response.facet_counts.facet_fields[this.field]), max = (data.length > this.maxDisplay) ? this.maxDisplay
            : data.length, elm = $(this.elm);
        if (data.length == 0) { // if no data to display, hide the widget
          ...
          for (var i = 0; i < max; i++) { // For each facet result, determines the level depth, creates the corresponding level object and stock it in
                                          // the LevelsHashMap
            ...
            if (levelDepth == this.rootLevel) { // it is a root level (it's depth corresponds to the defined rootLevel depth)
            ...
            } else if (this.rootLevel < levelDepth && levelDepth < realMaxDepth) { // check that the depth is allowed (between the rootLevel depth
              ...
              var parentLevelChildrenArray = this.LevelsHashMap["level" + (levelDepth - 1)][parentLevel].array; // retrieve the parent level children
                                                                                                                // list
            ...
            if (this.checkedOnTop === true) {
              ...
              elm.find('ul li .filterFacetCheck input:checked').parents('li').each(
                  function() {
                    ...
                    $(this).nextAll().each(
                        function() {
                          if ($('.filterFacetCheck .filterFacetLinkValue', $this).text().toLowerCase() > $('.filterFacetCheck .filterFacetLinkValue',
                              this).text().toLowerCase()
                              && $('.filterFacetCheck input', this).attr('checked') == 'checked')
                            $(this).after($($this).detach());
                        });
                  });
              elm.find('ul li .filterFacetCheck input:not(:checked)').parents('li').each(
                  function() {
                    var $this = this;
                    $(this).nextAll().each(
                        function() {
          ...
          case 'occurences':
            elm.find('ul li').each(
                function() {
                  ...
                  $(this).nextAll().each(
                      function() {
                        if (parseInt($($this).children('label').find('.filterFacetLinkCount span').text()) < parseInt($(this).children('label').find(
                            '.filterFacetLinkCount span').text())) {
                          $(this).after($($this).detach());
                        }
                      });
                });
      ...
      displayLevel : function(level, levelName, levelDepth, widget, currentUl) {
        ...
        var count = level.nb;
        if (levelDepth > widget.rootLevel) { // trick to keep the entire name of root levels but only the last part for non root levels (matter of
                                              // label display)
        ...
        currentLi.find('.filterFacetCheck input').change(
            function() { // sets the onChange function of the checkbox
              if ($(this).attr('checked') == 'checked') {
                // the checkbox is checked so the value must be added as a filter of the search query

                // optional functionality, depending on the chosen facet behavior (must be commented if a selected facet must not includes all
                // children)
                $(this).closest('li').children('ul').find('li').each(
                    function() { // if the current checkbox has children, removes all the children filters from the search query as the parent filter
                                  // covers the children ones
                      if ($(this).children('label').find('.filterFacetCheck input').attr('checked') == 'checked') {
                        widget.unselectNoRequest($(this).children('label').find('.filterFacetCheck input').val().replace(
                            /[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&"));
                      }
                    });
                ...
                widget.clickHandler();
                widget.selectHandler($(this).val().replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&")); // add the checkbox value as a filter for
                                                                                                            // the search query
              } else {
                ...
                $(this).parents('ul').each(
                    function() { // if the parents of the checkbox are checked, removes the parents values from the search query filters and adds the
                      ...
                      if ($(this).prev('label').find('.filterFacetCheck input').attr('checked') == 'checked') {
                        widget.unselectNoRequest($(this).prev('label').find('.filterFacetCheck input').val().replace(
                            /[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&")); // removes the parent value from the search query filters
                        ...
                        $(this).children('li').each(
                            function() {
                              ...
                              if ($(this).children('label').find('.filterFacetCheck input').attr('checked') == 'checked') {
                                widget.selectNoRequest($(this).children('label').find('.filterFacetCheck input').val().replace(
                                    /[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&"));
                              }
                            });
                      }
