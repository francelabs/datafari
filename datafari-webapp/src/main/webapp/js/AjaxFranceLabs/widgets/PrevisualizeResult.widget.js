		...
		if (docs.length!=0){
			$.each(docs,function(index,doc){
				...
				if (docs[index].last_modified != undefined){
					last_modified = docs[index].last_modified;
				}

				if (docs[index].crawl_date != undefined){
					crawl_date = docs[index].crawl_date;
				}
				...
				if (docs[index].file_size != undefined){
					author = docs[index].file_size;
				}
				...
				htmlContent += '<tr><td style="width:170px">'+author+'</td><td>'+file_size+'</td></tr>';			
				htmlContent += '<tr><td></td><td></td></tr>';			
