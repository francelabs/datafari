 ...
 * under the License.
 ****************************************************************************************************************************************************/
AjaxFranceLabs.AdvancedSearchWidget = AjaxFranceLabs.AbstractWidget
    .extend({
      ...
      // Be careful : This number is never decreased, even if a field is deleted by the user or removed by the onchange method of the dropdown box to
      // select a field !
      // Therefore it must not be used to determine the current number of effective fields
      fieldNumber : 0,

      // This is the list of AdvancedSearchFields objects that have been instanciated and currently effective
      // The ones that are deleted by the user or removed by the onchange method of the dropdown box to select a field are and must be removed from
      // this list !
      fieldsList : [],

      // This is an object containing mapping values for fieldnames
      // It is only used to replace matching fieldnames by their mapping values in the labels displayed to the user
      mappingFieldNameValues : {},

      // Represents the list of fieldnames that have a specific Solr field for exact match queries associated with the name of this specific Solr
      // field.
      // The exact match Solr field must be formated like this : [original_fieldname]_exact
      exactFieldsList : null,

      // This is an object containing fields that must have the autocomplete functionality. The fields are mapped to their Solr suggester's name that
      // will be used to perform suggest requests
      autocompleteFields : {},

      // This is an object containing fields that must have a list of fixed values instead of a free text input. The fields are mapped to their list
      // of fixed values values
      fixedValuesFields : {},
      ...
      buildWidget : function() {
        ...
        $.get("./GetExactFields", function(data) {
          if (data.code == 0) {
            ...
            // Construct the exactFieldsList using a fieldname as key and [fieldname]_exact as the corresponding value that represents the Solr
      ...
      buildStartingUI : function() {
        ...
        var baseSearch = "";
        // Extract the base search which is everything before the index of the first field filter or everything at all if not field filter is found
        if (indexOf != -1) {
        ...
        if (baseExactExpr != null && baseExactExpr != undefined && baseExactExpr != "") {
          ...
          if (exactContentExpr != null && exactContentExpr != undefined && exactContentExpr != "" && exactTitleExpr != null
              && exactTitleExpr != undefined && exactTitleExpr != "") {
        ...
        // Build the title
        this.advTable.append('<div class="head-title"><span id="advancedSeach-label"></span></div>');
        $("#advancedSeach-label").append(window.i18n.msgStore['advancedSearch-label']);
        ...
        this.advTable.append('<div><span id="add_adv_field" title="' + window.i18n.msgStore['advancedSearch-add-filter-tooltip']
            + '" class="add-field-button">+</span></div>');
        $('#add_adv_field').click(function() {
          self.addField();
        });

        // Build the exec search button
        this.advTable.append('<button id="exec_adv_search">' + window.i18n.msgStore['advancedSearch-makesearch-btn'] + '</button>').find('button')
            .click(function() {
              self.makeRequest();
            });

        // Add other fields filters if the base/original query searched by the user is not empty
        if (baseQuery != "" && baseQuery != "*" && baseQuery != "*:*") {
          ...
          // Object which will contain the last found field filter that corresponds to a Solr field used for exact match (ie [field]_exact)
          var lastExact = {};
          if (fields != null && fields != undefined && fields != "") {
            // For each field filter, the operator (if any), the fieldname and the filter values are extracted and the corresponding UI is built
            for (var cpt = 0; cpt < fields.length; cpt++) {
              ...
              // If the first char of the fieldname is -, it is because it is a negative filter
              // need to remove this char to get the real fieldname
              if (fieldname.startsWith("-")) {
                fielname = fieldname.substring(1);
                negativeExpression = true;
              }
              // Remove the fielname from the fieldExpression as it only remains the filter expression
              ...
              if (fieldname.endsWith("_exact") && (fieldExpression.startsWith("\"") || fieldExpression.startsWith("(\""))
              ...
              if (fieldType == "tdate" || fieldType == "date") {
                fieldType = "date";
              } else if (fieldType == "int" || fieldType == "long" || fieldType == "float" || fieldType == "double" || fieldType == "tint"
                  || fieldType == "tlong" || fieldType == "tfloat" || fieldType == "tdouble") {
                fieldType = "num";
              } else {
                fieldType = "text";
              }
              ...
              if (fieldType == "text") {
                ...
                // If the field type is not text but a number or a date then the filter value can only have two shapes : a single value or a range of
                // values
                var fromValue = null;
                var toValue = null;
                // Detect if the filter value is a range and extract the from and to values if it is the case
                if (fieldExpression.startsWith('[')) {
                  ...
                  // Thus to simplify the treatments of the advanced search, the direct value is converted to a range filter with the same value as
                  // from/to values
                  fromValue = fieldExpression;
                  toValue = fieldExpression;
                }
                // Set the extractedValues variable with the extracted from/to values
                extractedValues = {
                  "fromValue" : fromValue,
                  "toValue" : toValue
                };
              ...
              // otherwise its exact filter value needs to be saved for the next iteration in order to either add it to it's corresponding "normal"
            ...
            if (!jQuery.isEmptyObject(lastExact)) {
              // The lastExact object is not empty
              // It was not added to the UI because it was the last field of the for loop and thus needs to be added now !
              var lastExactOriginalFieldname = "";
              for ( var key in lastExact) {
                lastExactOriginalFieldname = key;
              }
      ...
      extractFilterFromText : function(text, negativeExpression, isBasicSearchText) {
        ...
        if (!text.startsWith("(") && !isBasicSearchText) {
          // The filter value is a direct value, no needs to apply complex treatment to extract multiple filters
          ...
          if (exactNegativeExpressions != null && exactNegativeExpressions != undefined) {
            for (var cptNgtExact = 0; cptNgtExact < exactNegativeExpressions.length; cptNgtExact++) {
              ...
              // If there is a space in front of the exact expression then remove it + the exact expression, otherwise only remove the exact
              // expression
          ...
          var tempFieldExpression = text; // initialize a temp variable because at the end of this OR expression extraction,
          // the original text variable must stay untouched for the AND regex to work
          ...
          tempFieldExpression = text; // reinit the temp variable with the text variable which has not been modified by the previous OR expression
          // extraction
          ...
          // -----------------------------------------------------------------------------------------------------------------------------------------
          // NB: the following cleaning process of the OR and AND expression seems weird and ugly but after bunch of tests,
          // it appears that it needs to be done this way, in that specific order to work like a charm !
      ...
      // if called with no fieldName as param it just builds a dropdown box to select a field and implements its onchange method
      // otherwise builds the dropdown box and initialize it with the provided fielName param and trigger the constructFilter method that will build
      // the correct filters in the UI according to the field type and eventually fill the inputs with provided values if any
      addField : function(fieldName, values, operator) {
        // Deactivate tooltips
        this.elm.tooltip('disable');

        var self = this;
        // Builds the dropdown box of the operator and the dropdown box of the available fields
        ...
        currentDiv
            .append('<select class="select-operator dropdown"><option selected value="AND">AND</option><option value="OR">OR</option></select><select class="dropdown-field dropdown" id="'
                + selectID
                + '"><option disabled selected value>'
                + window.i18n.msgStore['selectField']
                + '</option></select><span class="delete_button button"> X </span>');
        ...
        for (var i = 0; i < this.available_fields.field.length; i++) {
          ...
          select.append('<option value="' + this.available_fields.field[i].name + '" type="' + this.available_fields.field[i].type + '">'
        ...
        select.change(function() {
          ...
          var type = $("#" + selectID + " option:selected").attr("type"); // Retrieve the type of the selected field
          var field = $("#" + selectID + " option:selected").attr("value"); // Retrieve the fieldname of the selected field
          var selectSave = select.detach(); // Detach the dropdown box of fields as it is saved it with all the assigned attributes and javascript
          // methods
          var selectOperatorSave = selectOperator.detach(); // Detach the dropdown box of operator as it is saved it with all the assigned attributes
          // and javascript methods
          var deleteButtonSave = deleteButton.detach(); // Detach the delete button as it is saved with all its attributes and javascript methods
          ...
          var newOperator = div.find('.select-operator'); // Retrieve the correct operator dropdown box because the reference changes after each
          // detach/attach jquery methods
          self.constructFilter(type, div, field, null, newOperator, currentFieldNum); // Builds the correct UI associated to the new selected field
        });
      ...
      beforeRequest : function() {
        ...
        if ($(this.elm).is(':visible') && !this.manager.store.isParamDefined('original_query')) {
          ...
          // Clean finalFilter : remove the operator that starts the final filter as it is useless
          ...
          // If the finalFilter is empty then replace it by the 'search everything' expression '*:*' and let the search bar empty
      ...
      updateAddressBar : function() {
        ...
        window.history.pushState('Object', 'Title', window.location.pathname + '?query=' + this.manager.store.get('q').val() + '&lang='
