    ...
    if (!this.initialized) {
      ...
      var opt = {
        ...
        source : function(request, response) {
          ...
          // Abort any previously triggered request
          ...
          if (AjaxFranceLabs.extractLast(request.term) != '') {
            self.lastXhr = self.manager.executeRequest(self.connectionInfo.serverUrl, self.connectionInfo.servlet, self.connectionInfo.queryString
                + encodeURIComponent('' + AjaxFranceLabs.extractLast(request.term) + '') + f + "&autocomplete=true" + "&spellcheck.collateParam.q.op=" + self.manager.store.get("q.op").val(),
                function(data, status, xhr) {
                  var src = [];
                  // self.manager.executeRequest('',
                  // '', 'q=' +
