//# sourceURL=/Datafari/resources/js/admin/ajax//modifyUsers.js
...
function setupLanguage() {
  ...
  $("#documentation-modifyusers").text(window.i18n.msgStore['documentation-modifyusers']);
}
...
function htmlRole(role, username) {
  return '<div class="inline_block ' + username + '" data-user="' + username + '"><div class="input-group role">' + '<span class="input-group-addon delete notyet"><i class="fas fa-times"></i></span>'
      + '<input class="form-control ' + role + '" value="' + role + '" type="text" disabled/>' + '</div></div>';
}
...
if (window.dialogue == undefined) {
  window.dialogue = $("#delete-dialog").dialog({
    autoOpen : false,
    buttons : [ {
      text : "Delete",
      icon : "  ui-icon-check",
      click : function() {
        ...
        $.post("../SearchAdministrator/deleteUser", {
          username : elementDiv.data("user")
        }, function(data) {
          if (data.code == SERVERALLOK) {
            elementDiv.remove();
          } else {
...
$.get("../SearchAdministrator/getAllUsersAndRoles", {
  usersType : "Datafari"
}, function(data) {
  if (data.code == SERVERALLOK) {
    ...
    $.each(users, function(index, user) {
      ...
      for (var i = 0; i < roles.length; i++) {
        html += htmlRole(roles[i], user.username);
      }

      html += '<input data-user="' + user.username + '" class="add_role" placeHolder="Add a Role"/></th>' + '<th><span class="delete_user"><i class="fas fa-times"></i></span></th>' + '</tr>';
    });
    $("tbody").append(html);
    $(".password").keydown(function(e) {
      if (e.keyCode == 13) {
        ...
        $.post("../SearchAdministrator/changePassword", {
          username : element.data("user"),
    ...
    $(".delete_user").click(function(e) {
      ...
      $.get("../SearchAdministrator/checkUser", {
        username : elementDiv.data("user"),
        action : "delete"
      }, function(data) {
        if (data.code == SERVERALLOK) {
    ...
    $(".add_role").autocomplete({
      ...
      select : function(event, ui) {
        ...
        if ($("." + element.data("user") + " ." + role).length == 0) {
          $.post("../SearchAdministrator/addRole", {
            username : element.data("user"),
            role : role
          }, function(data) {
            if (data.code == SERVERALLOK) {
