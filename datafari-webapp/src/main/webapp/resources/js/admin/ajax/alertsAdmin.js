      ...
      .ajax({ // Ajax request to the doGet of the AlertsAdmin servlet
        ...
        success : function(data, textStatus, jqXHR) {
          ...
          if (data.toString().indexOf("Error code : ") !== -1) {
            ...
            $("#mailForm").append("<fieldset id=\"fieldForm\" >");
            $("#fieldForm")
                .append(
                    "<legend id=\"mailLegend\">"
                        + window.i18n.msgStore["mailConf"]
                        + "</legend><div class=\"row\"><label id=SMPTLabel class=\"col-sm-3 col-form-label\">SMTP : </label><input type=\"text\" class=\"col-sm-2\" id=\"SMTP\"  style=\"min-width : 150px;\" value="
                        + "\"" + data.smtp + "\"" + "></div>");
            $("#fieldForm").append(
                "<div class=\"row\"><label id=AddressLabel class=\"col-sm-3 col-form-label\">" + window.i18n.msgStore["address"] + "</label><input type=\"text\" title=\""
                    + window.i18n.msgStore["address-tooltip"] + "\" class=\"col-sm-2\" id=\"Address\" style=\"min-width : 150px;\"  value=" + "\"" + data.from + "\"" + "></div>");
            $("#fieldForm").append(
                "<div class=\"row\"><label id=UserLabel class=\"col-sm-3 col-form-label\">" + window.i18n.msgStore["user"] + "</label><input type=\"text\" title=\""
                    + window.i18n.msgStore["user-tooltip"] + "\" class=\"col-sm-2\" id=\"UserName\" style=\"min-width : 150px;\"  value=" + "\"" + data.user + "\"" + "></div>");
            $("#fieldForm").append(
                "<div class=\"row\"><label id=PassLabel class=\"col-sm-3 col-form-label\">" + window.i18n.msgStore["pass"]
                    + "</label><input type=\"password\" class=\"col-sm-2\" id=\"Pass\" style=\"min-width : 150px;\"  value=" + "\"" + data.pass + "\"" + "></div>");
...
function parameters() {
  var data = "HOURLYDELAY=" + document.getElementById("HourlyDelay").value + "&DAILYDELAY=" + document.getElementById("DailyDelay").value + "&WEEKLYDELAY="
      + document.getElementById("WeeklyDelay").value + "&HOST=" + document.getElementById("Host").value + "&PORT=" + document.getElementById("Port").value + "&DATABASE="
      + document.getElementById("Database").value + "&COLLECTION=" + document.getElementById("Collection").value + "&smtp=" + document.getElementById("SMTP").value + "&from="
