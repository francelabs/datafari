...
AjaxFranceLabs.AbstractManager = AjaxFranceLabs.Class.extend({
  ...
  init : function() {
    ...
    if (this.constellio) {
      ...
      for ( var externalSource in self.externalSources) {
        self.externalSources[externalSource].init();
      }
      return $.Deferred().resolve;
    }
  },
  ...
  handleError : function(jqxhr, status, error) {
    for ( var widget in this.widgets) {
      this.widgets[widget].requestError(status, error, jqxhr);
    }
    for ( var module in this.modules) {
      this.modules[module].requestError(status, error, jqxhr);
    }
  },

  makeRequest : function(servlet, pagination) {
    var self = this;
