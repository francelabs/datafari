...
AjaxFranceLabs.SpellcheckerWidget = AjaxFranceLabs.AbstractWidget.extend({
  ...
  afterRequest : function() {
    ...
    if (data.response.numFound == 0 && data.spellcheck !== undefined && data.spellcheck.collations.length > 0) {
      ...
      /*
       * Store the "original" searched value We use this function instead of self.manager.store.add('original_query', ...); to have an
       * object of type subclass inside original_query instead of string. Otherwise we get exception in
       * params.push(this.params[name].string()); of ParametersStore.js
       */
      ...
      $(self.elm).append(
          '<span class="spellcheckerResult">' + window.i18n.msgStore['spellcheckerResults'] + '<span class="result">' + data.spellcheck.collations[1] + '</span></span><br/><br/><span>'
              + window.i18n.msgStore['notHitsQueries'] + '<span class="result">' + self.manager.store.get('original_query').val() + '</span></span>');

      /*
       * Take the collation value: first value of array contains "collation", this is why we get [1] instead of [0] Perform a new query with
       * the corrected value
       */
      ...
      if (self.manager.store.isParamDefined('original_query') !== true) {
        ...
        /*
         * We just performed a new query with a corrected value by spellchecker => reset the manager.store to be ready for the next query
         * from the user
         */
        self.manager.store.remove('original_query');
      }
  ...
  doSpellcheckerQuery : function(correctedValue) {
    ...
    /*
     * Do not update the searchBar widget linked to the current manager with the new searched value: we want to display the misspelled
     * search term for ( var w in self.manager.widgets) { if (self.manager.widgets[w].type === 'searchBar') {
