 ...
 ******************************************************************************************************************************************/
AjaxFranceLabs.FacetDuplicates = AjaxFranceLabs.TableWidget
    .extend({
      ...
      update : function() {
        ...
        if (data.length !== 0) {
          ...
          $.get(self.manager.serverUrl + 'select?q=*%3A*&rows=1&fl=title&wt=json&indent=true&hl=off&_=1433765381298&fq=signature%3A%22' + data[compteur].name + '%22',
              function recursive(responseJSON) {
                ...
                if (compteur < data.length)
                  ...
                  for (var i = 0; i < max; i++) {
                    if (names[i] !== '') {
                      ...
                      if (self.manager.store.find('fq', new RegExp(self.field + ':'
                      ...
                      if (elm.find('ul li:last .filterFacetCheck input').is(':checked'))
                        elm.find('ul li:last .filterFacetCheck label').attr('for', data[i].name).append(
                            '<span class="checkboxIcon far fa-check-square">&nbsp;</span>' + '<span class="filterFacetLinkValue">' + AjaxFranceLabs.tinyString(names[i], 19) + '</span>').append(
                            '&nbsp;<span class="filterFacetLinkCount">(<span>' + data[i].nb + '</span>)</span>');
                      else
                        elm.find('ul li:last .filterFacetCheck label').attr('for', data[i].name).append(
                            '<span class="checkboxIcon far fa-square">&nbsp;</span>' + '<span class="filterFacetLinkValue">' + AjaxFranceLabs.tinyString(names[i], 19) + '</span>').append(
                            '&nbsp;<span class="filterFacetLinkCount">(<span>' + data[i].nb + '</span>)</span>');
                    }
                  }
                  if (self.pagination) {
