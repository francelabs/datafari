            ...
            for (var i = 0; i < max; i++) {
                ...
                if (decodedName !== '' && $.inArray(decodedName, self.discardedValues) == -1) {
                    ...
                    elm.find('ul li:last .filterFacetCheck label')
                        .append('<span class="filterFacetLinkValue"><a href="/Datafari/Search?lang=' + window.i18n.language + '&request=' + encodeURIComponent(query) + '">' + 
                            AjaxFranceLabs.tinyString(label, self.displayChars) + '(' + Math.floor(data[i].nb * 100/ this.total) + ')' + '</a></span>')
