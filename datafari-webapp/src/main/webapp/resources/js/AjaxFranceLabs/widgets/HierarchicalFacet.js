...
AjaxFranceLabs.HierarchicalFacetWidget = AjaxFranceLabs.AbstractFacetWidget.extend({
  ...
  buildWidget : function() {
    ...
    if (this.name != null) {
      ...
      elm.find('.facetName').click(function() {
        $('.facetSort, ul, .pagerModule.show', $(this).parents('.hierarchicalWidget')).toggle();
  ...
  displayLevel : function(level, levelName, levelDepth, widget, currentUl) {
    ...
    if (currentUl.prev('label').find('.filterFacetCheck input').is(':checked')) { // check if the parent level checkbox is
      // checked
    ...
    currentLi.find('.filterFacetCheck input').change(function() { // sets the onChange function of the checkbox
      if ($(this).is(':checked')) {
        ...
        // children)
        $(this).closest('li').children('ul').find('li').each(function() { // if the current checkbox has children, removes all the children
        ...
        // optional functionality, depending on the chosen facet behavior (must be commented if a selected facet must not includes all
        // children)
        $(this).parents('ul').each(function() { // if the parents of the checkbox are checked, removes the parents values from the search
