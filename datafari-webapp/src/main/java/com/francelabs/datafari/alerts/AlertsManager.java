...
public class AlertsManager {
  ...
  /**
   * Gets the path of datafari.properties file Reads the file to fill the variables if the ALERTS line was set to true, then it establishes
  ...
  /**
   * Creates the runnables Calculates the delays if necessary Schedules the runnables
   *
   * @throws IOException
   */
   ...
   *
   * @param custom
   *          the customBool to know if alerts of this frequency have been launched
   * @param delay
   ...
   */
  public ScheduledFuture<?> launch(final AlertFrequencyFirstExecution custom, final DateTime delay, ScheduledFuture<?> Handle, final DateTime current, final String frequency, final String Hour,
      final Runnable run, final long loop) {
    try {
  ...
  /**
   * Updates the datafari.properties file Run the alerts with the correct frequency
   *
   * @param frequency
   *          : Hourly/Daily/Weekly
   */
    ...
    try {
      ...
      for (final Properties alertProp : alertList) { // Get the next
        ...
        if (frequency.toLowerCase().equals(alertProp.get("frequency").toString().toLowerCase())) {
          ...
          alert = new Alert(alertProp.get("subject").toString(), alertProp.get("mail").toString(), server, alertProp.get("keyword").toString(), alertProp.get("filters").toString(),
              alertProp.get("frequency").toString(), mail, alertProp.get("username").toString());
  ...
  /**
   * Gets the parameters Establishes the connection to the database Starts the alerts
   *
   * @throws IOException
   * @throws ParseException
   */
  ...
  /**
   * Closes the database connection Cancels the scheduled runnables Resets various variables
   */
  public void turnOff() {
    LOGGER.debug("Stopping Alert scheduler...");
  ...
  /**
   * Calculate the difference in minutes between the current date time and the provided scheduled one, according to the frequency
   *
   * @param frequency
   *          the frequency of the scheduled date time
   * @param scheduledDate
   *          the initial scheduled date time that the user has typed in the admin UI
   * @return the difference in minutes between the current date time and the next scheduled one
   */
