 ...
 * Javadoc
 *
 * This servlet is used to modify or download the Protwords files of the FileShare core It is only called by theProtWords.html. doGet is
 * used to print the content of the file or download it. doPost is used to confirm the modifications of the file. The semaphore is created
 * in the constructor.
 *
 *
 */
...
public class HelpAdmin extends HttpServlet {
  ...
  /**
   * @throws IOException
   * @see HttpServlet#HttpServlet() Gets the list of the languages Creates a semaphore for each of them
   */
  public HelpAdmin() throws IOException {
    ...
    env = environnement + File.separator + "tomcat" + File.separator + "webapps" + File.separator + "Datafari" + File.separator + "resources" + File.separator + "helpAssets";
  }

  /**
   * @throws IOException
   * @see HttpServlet#doGet(HttpServletRequest request, HttpServletResponse response) used to print the content of the file or to download it
  ...
  protected void doGet(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
    try {
      ...
      try {
        ...
        final PrintWriter out = response.getWriter();
        out.append("Error while reading the help page file, please make sure the file exists and retry, if the problem persists contact your system administrator. Error code : 69014");
        out.close();
  ...
  /**
   * @throws IOException
   * @see HttpServlet#doPost(HttpServletRequest request, HttpServletResponse response) used to confirm the modifications and/or release the
   *      semaphore called by the confirm modification or if the user loads an other page, refreshes the page or switches language
   */
  @Override
    ...
    try {
      if (request.getParameter("content") == null) {
        ...
        try {
          ...
          final PrintWriter out = response.getWriter();
          out.append("Error while rewriting the protwords file, please make sure the file exists and retry, if the problem persists contact your system administrator. Error code : 69015");
          out.close();
