...
# Solr 10+: zkcli.sh is removed, use bin/solr zk <cmd>
# bin/solr zk upconfig -z <zkHost> -n <confName> -d <confDir>
SOLR_ZK_CLI="$DATAFARI_HOME/solr/bin/solr"
if [ ! -x "$SOLR_ZK_CLI" ]; then
  echo "ERROR: Solr CLI not found or not executable: $SOLR_ZK_CLI"
  echo "Expected path: \$DATAFARI_HOME/solr/bin/solr"
  exit 1
fi

echo "Helper script to load or push Solr configuration to Zookeeper"
if [ "$NODETYPE" != "monoserver" ] && [ "$NODETYPE" != "main" ]; then
...
if [ "$NODETYPE" = "monoserver" ]; then
  ...
  value="\<${conf_name}\>"
  if [[ ${array_analytics_collections[@]} =~ $value ]]; then
...
if [ "$conf_action" = "upload" ]; then
  default_upload_folder="/opt/datafari/solr/solrcloud/$conf_name/conf"
  read -p "What is the location of the folder that contains your configuration to upload ? [$default_upload_folder]" upload_folder
  upload_folder=${upload_folder:-$default_upload_folder}
  echo "$upload_folder"

  # Solr 10+ replacement for zkcli.sh -cmd upconfig
  # bin/solr zk upconfig -z <zkHost> -n <confName> -d <confDir>
  "$SOLR_ZK_CLI" zk upconfig -z "$zk_ip_script" -d "$upload_folder" -n "$conf_name"
...
elif [ "$conf_action" = "download" ]; then
  ...
  # Solr 10+ replacement for zkcli.sh -cmd downconfig
  # bin/solr zk downconfig -z <zkHost> -n <confName> -d <targetDir>
  "$SOLR_ZK_CLI" zk downconfig -z "$zk_ip_script" -d "$download_folder" -n "$conf_name"
fi
