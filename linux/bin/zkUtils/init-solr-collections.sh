...
create_collection() {
    ...
    curl -XPOST --insecure -H 'Content-type:application/json' -d '{"set-user-property": {"texttagger.host": "localhost:2181"}}' $url_protocol://${ip_solr}/solr/$1/config
  	...
  	curl -XPOST --insecure -H 'Content-type:application/json' -d '{"set-property": {"supdateHandler.autoCommit.maxTime": "300000"}}' $url_protocol://${ip_solr}/solr/$1/config
...
curl -XPOST --insecure -H 'Content-type:application/json' -d '{"set-user-property": {"duplicates.quant.rate": "0.1"}}' $url_protocol://${ip_solr}/solr/Duplicates/config
  ...
  curl -XPOST --insecure -H 'Content-type:application/json' -d '{"set-property": {"updateHandler.autoCommit.maxTime": "600000"}}' $url_protocol://${ip_solr}/solr/Statistics/config
  curl -XPOST --insecure -H 'Content-type:application/json' -d '{"set-property": {"updateHandler.autoCommit.maxTime": "600000"}}' $url_protocol://${ip_solr}/solr/Promolink/config
  curl -XPOST --insecure -H 'Content-type:application/json' -d '{"set-property": {"updateHandler.autoCommit.maxTime": "600000"}}' $url_protocol://${ip_solr}/solr/Access/config
  curl -XPOST --insecure -H 'Content-type:application/json' -d '{"set-property": {"updateHandler.autoCommit.maxTime": "600000"}}' $url_protocol://${ip_solr}/solr/Monitoring/config
  curl -XPOST --insecure -H 'Content-type:application/json' -d '{"set-property": {"updateHandler.autoCommit.maxTime": "600000"}}' $url_protocol://${ip_solr}/solr/Duplicates/config
  ...
  curl -XPOST --insecure -H 'Content-type:application/json' -d '{"set-property": {"updateHandler.autoCommit.maxTime": "600000"}}' $url_protocol://${ip_solr}/solr/GenericAnnotator/config
