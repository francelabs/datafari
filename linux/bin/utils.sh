...
check_java()
{
  ...
  DATAFARI_ENV_FILE="${DATAFARI_HOME}/tomcat/conf/datafari-env.sh"
  JAVA_BIN=""

  # 1) DATAFARI_JAVA_HOME (admin / installer override)
  if [ -n "${DATAFARI_JAVA_HOME:-}" ] && [ -x "$DATAFARI_JAVA_HOME/bin/java" ]; then
    JAVA_HOME="$DATAFARI_JAVA_HOME"
    JAVA_BIN="$JAVA_HOME/bin/java"
    echo "Using DATAFARI_JAVA_HOME: $JAVA_HOME"
  ...
  # 3) java in PATH
    ...
    echo "ERROR: No Java detected. Please install Java 21." >&2
    exit 1
  fi

  # Safety check
  if [ ! -x "$JAVA_BIN" ]; then
    echo "ERROR: Java binary not executable: $JAVA_BIN" >&2
    exit 1
  fi

  # Version check (Java 21 only)
  version="$("$JAVA_BIN" -version 2>&1 | awk -F '"' '/version/ {print $2}' | awk -F '.' '{print $1}')"
  echo "Java version detected: $version"

  if [ "$version" != "21" ]; then
    echo "ERROR: Java $version detected, but Datafari requires Java 21." >&2
    echo "See: https://datafari.atlassian.net/wiki/spaces/DATAFARI/pages/110788634/Software+requirements" >&2
    exit 1
  fi
  ...
  export JAVA_HOME
  export DATAFARI_JAVA_HOME="$JAVA_HOME"

  # Persist override for Monit / non-interactive contexts
  if [ ! -f "$DATAFARI_ENV_FILE" ]; then
    cat > "$DATAFARI_ENV_FILE" <<EOF
# ---------------------------------------------------------------------------
# Datafari Java environment override
# Generated by init-datafari.sh
# ---------------------------------------------------------------------------
export DATAFARI_JAVA_HOME="$JAVA_HOME"
EOF
    chmod 644 "$DATAFARI_ENV_FILE"
    echo "Persisted DATAFARI_JAVA_HOME to $DATAFARI_ENV_FILE"
  else
    echo "datafari-env.sh already exists, leaving unchanged"
  fi
